...
contract AaveProtectionCallback is AbstractCallback, RescuableBase {
    ...
    struct ProtectionConfig {
        ...
        uint8 executionCount;        // total lifetime successful executions (informational)
        uint8 consecutiveFailures;   // resets to 0 on each success
    ...
    function _checkAndProtectConfig(uint256 configId) external returns (bool) {
        ...
        if (protectionExecuted) {
            ...
            config.consecutiveFailures = 0; // reset on success
        } else {
    ...
    function _performDebtRepayment(
        ...
        ).getUserReserveData(config.debtAsset, owner);
     ...
     * @return isActive Whether protection is active
     * @return protectionType Type of protection
     * @return healthFactorThreshold Health factor below which protection triggers
     * @return targetHealthFactor Target health factor to achieve
     * @return collateralAsset Asset to use for collateral deposits
     * @return debtAsset Asset to use for debt repayment
     * @return preferDebtRepayment If BOTH type, which method to prefer
     */
    ...
    {
        // Return latest active config or empty
